...
async def create_app():
    ...
    app.add_api_route("/langflow/files/upload", langflow_files.upload_user_file, methods=["POST"])
    app.add_api_route("/langflow/ingest", langflow_files.run_ingestion, methods=["POST"])
    app.add_api_route("/langflow/files", langflow_files.delete_user_files, methods=["DELETE"])
    app.add_api_route("/langflow/upload_ingest", langflow_files.upload_and_ingest_user_file, methods=["POST"])

    # Upload endpoints
    app.add_api_route("/upload_context", upload.upload_context, methods=["POST"])
    app.add_api_route("/upload_path", upload.upload_path, methods=["POST"])
    app.add_api_route("/upload_options", upload.upload_options, methods=["GET"])
    app.add_api_route("/upload_bucket", upload.upload_bucket, methods=["POST"])

    # Task endpoints
    app.add_api_route("/tasks/{task_id}", tasks.task_status, methods=["GET"])
    app.add_api_route("/tasks", tasks.all_tasks, methods=["GET"])
    app.add_api_route("/tasks/{task_id}/cancel", tasks.cancel_task, methods=["POST"])

    # Search endpoint
    app.add_api_route("/search", search.search, methods=["POST"])

    # Knowledge Filter endpoints
    app.add_api_route("/knowledge-filter", knowledge_filter.create_knowledge_filter, methods=["POST"])
    app.add_api_route("/knowledge-filter/search", knowledge_filter.search_knowledge_filters, methods=["POST"])
    app.add_api_route("/knowledge-filter/{filter_id}", knowledge_filter.get_knowledge_filter, methods=["GET"])
    app.add_api_route("/knowledge-filter/{filter_id}", knowledge_filter.update_knowledge_filter, methods=["PUT"])
    app.add_api_route("/knowledge-filter/{filter_id}", knowledge_filter.delete_knowledge_filter, methods=["DELETE"])

    # Knowledge Filter Subscription endpoints
    app.add_api_route("/knowledge-filter/{filter_id}/subscribe", knowledge_filter.subscribe_to_knowledge_filter, methods=["POST"])
    app.add_api_route("/knowledge-filter/{filter_id}/subscriptions", knowledge_filter.list_knowledge_filter_subscriptions, methods=["GET"])
    app.add_api_route("/knowledge-filter/{filter_id}/subscribe/{subscription_id}", knowledge_filter.cancel_knowledge_filter_subscription, methods=["DELETE"])

    # Knowledge Filter Webhook endpoint (no auth required - called by OpenSearch)
    app.add_api_route("/knowledge-filter/{filter_id}/webhook/{subscription_id}", knowledge_filter.knowledge_filter_webhook, methods=["POST"])

    # Chat endpoints
    app.add_api_route("/chat", chat.chat_endpoint, methods=["POST"])
    app.add_api_route("/langflow", chat.langflow_endpoint, methods=["POST"])

    # Chat history endpoints
    app.add_api_route("/chat/history", chat.chat_history_endpoint, methods=["GET"])
    app.add_api_route("/langflow/history", chat.langflow_history_endpoint, methods=["GET"])

    # Session deletion endpoint
    app.add_api_route("/sessions/{session_id}", chat.delete_session_endpoint, methods=["DELETE"])

    # Authentication endpoints
    app.add_api_route("/auth/init", auth.auth_init, methods=["POST"])
    app.add_api_route("/auth/callback", auth.auth_callback, methods=["POST"])
    app.add_api_route("/auth/me", auth.auth_me, methods=["GET"])
    app.add_api_route("/auth/logout", auth.auth_logout, methods=["POST"])

    # Connector endpoints
    app.add_api_route("/connectors", connectors.list_connectors, methods=["GET"])
    app.add_api_route("/connectors/{connector_type}/sync", connectors.connector_sync, methods=["POST"])
    app.add_api_route("/connectors/sync-all", connectors.sync_all_connectors, methods=["POST"])
    app.add_api_route("/connectors/{connector_type}/status", connectors.connector_status, methods=["GET"])
    app.add_api_route("/connectors/{connector_type}/token", connectors.connector_token, methods=["GET"])
    app.add_api_route("/connectors/{connector_type}/disconnect", connectors.connector_disconnect, methods=["DELETE"])
    app.add_api_route("/connectors/{connector_type}/webhook", connectors.connector_webhook, methods=["POST", "GET"])

    # Document endpoints
    app.add_api_route("/documents/check-filename", documents.check_filename_exists, methods=["GET"])
    app.add_api_route("/documents/delete-by-filename", documents.delete_documents_by_filename, methods=["POST"])

    # OIDC endpoints
    app.add_api_route("/.well-known/openid-configuration", oidc.oidc_discovery, methods=["GET"])
    app.add_api_route("/auth/jwks", oidc.jwks_endpoint, methods=["GET"])
    app.add_api_route("/auth/introspect", oidc.token_introspection, methods=["POST"])

    # Settings endpoints
    app.add_api_route("/settings", settings.get_settings, methods=["GET"])
    app.add_api_route("/settings", settings.update_settings, methods=["POST"])
    app.add_api_route("/onboarding/state", settings.update_onboarding_state, methods=["POST"])

    # Provider health check endpoint
    app.add_api_route("/provider/health", provider_health.check_provider_health, methods=["GET"])
    ...
    # Models endpoints
    app.add_api_route("/models/openai", models.get_openai_models, methods=["POST"])
    app.add_api_route("/models/anthropic", models.get_anthropic_models, methods=["POST"])
    app.add_api_route("/models/ollama", models.get_ollama_models, methods=["GET"])
    app.add_api_route("/models/ibm", models.get_ibm_models, methods=["POST"])

    # Onboarding endpoints
    app.add_api_route("/onboarding", settings.onboarding, methods=["POST"])
    app.add_api_route("/onboarding/rollback", settings.rollback_onboarding, methods=["POST"])

    # Docling preset update endpoint
    app.add_api_route("/settings/docling-preset", settings.update_docling_preset, methods=["PATCH"])

    # Nudges endpoints
    app.add_api_route("/nudges", nudges.nudges_from_kb_endpoint, methods=["POST"])
    app.add_api_route("/nudges/{chat_id}", nudges.nudges_from_chat_id_endpoint, methods=["POST"])

    # Flow reset endpoint
    app.add_api_route("/reset-flow/{flow_type}", flows.reset_flow_endpoint, methods=["POST"])

    # Router upload ingest endpoint
    app.add_api_route("/router/upload_ingest", router.upload_ingest_router, methods=["POST"])

    # Docling service proxy
    app.add_api_route("/docling/health", docling.health, methods=["GET"])

    # ===== API Key Management Endpoints (JWT auth for UI) =====
    app.add_api_route("/keys", api_keys.list_keys_endpoint, methods=["GET"])
    app.add_api_route("/keys", api_keys.create_key_endpoint, methods=["POST"])
    app.add_api_route("/keys/{key_id}", api_keys.revoke_key_endpoint, methods=["DELETE"])

    # ===== Public API v1 Endpoints (API Key auth) =====
    # Chat endpoints
    app.add_api_route("/v1/chat", v1_chat.chat_create_endpoint, methods=["POST"])
    app.add_api_route("/v1/chat", v1_chat.chat_list_endpoint, methods=["GET"])
    app.add_api_route("/v1/chat/{chat_id}", v1_chat.chat_get_endpoint, methods=["GET"])
    app.add_api_route("/v1/chat/{chat_id}", v1_chat.chat_delete_endpoint, methods=["DELETE"])

    # Search endpoint
    app.add_api_route("/v1/search", v1_search.search_endpoint, methods=["POST"])

    # Documents endpoints
    app.add_api_route("/v1/documents/ingest", v1_documents.ingest_endpoint, methods=["POST"])
    app.add_api_route("/v1/tasks/{task_id}", v1_documents.task_status_endpoint, methods=["GET"])
    app.add_api_route("/v1/documents", v1_documents.delete_document_endpoint, methods=["DELETE"])

    # Settings endpoints
    app.add_api_route("/v1/settings", v1_settings.get_settings_endpoint, methods=["GET"])
    app.add_api_route("/v1/settings", v1_settings.update_settings_endpoint, methods=["POST"])

    # Models endpoint
    app.add_api_route("/v1/models/{provider}", v1_models.list_models_endpoint, methods=["GET"])

    # Knowledge filters endpoints
    app.add_api_route("/v1/knowledge-filters", v1_knowledge_filters.create_endpoint, methods=["POST"])
    app.add_api_route("/v1/knowledge-filters/search", v1_knowledge_filters.search_endpoint, methods=["POST"])
    app.add_api_route("/v1/knowledge-filters/{filter_id}", v1_knowledge_filters.get_endpoint, methods=["GET"])
    app.add_api_route("/v1/knowledge-filters/{filter_id}", v1_knowledge_filters.update_endpoint, methods=["PUT"])
    app.add_api_route("/v1/knowledge-filters/{filter_id}", v1_knowledge_filters.delete_endpoint, methods=["DELETE"])

    ...
    try:
        ...
        if IBM_AUTH_ENABLED:
            ...
            if os_claims:
                ...
                if os_jwt:
                    mcp_global_vars["JWT"] = os_jwt
                mcp_global_vars["OWNER"] = os_claims.get("user_id", os_claims.get("sub", ""))
                owner_name = os_claims.get("name", "")
                if owner_name:
                    mcp_global_vars["OWNER_NAME"] = f'"{owner_name}"'
                mcp_global_vars["OWNER_EMAIL"] = os_claims.get(
                    "email", os_claims.get("preferred_username", "")
                )
                mcp_global_vars["CONNECTOR_TYPE_URL"] = "url"
                logger.debug(
                    "Added OpenSearch JWT owner details to MCP servers for IBM auth mode",
                    owner=mcp_global_vars.get("OWNER"),
                    owner_email=mcp_global_vars.get("OWNER_EMAIL"),
                )

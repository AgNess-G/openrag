"""
Unit tests for chat filter → OpenSearch body merge helpers.

Imports ``opensearch_filter_merge_standalone`` from ``flows/components/`` (stdlib
only). The same logic is embedded in ``opensearch_multimodal.py`` for Langflow.
No OpenRAG ``src`` or ``utils`` imports; Langflow image does not need ``COPY src``.
"""
...
_COMPONENTS = Path(__file__).resolve().parents[2] / "flows" / "components"
if str(_COMPONENTS) not in sys.path:
    sys.path.insert(0, str(_COMPONENTS))

from opensearch_filter_merge_standalone import (
...
class TestApplyChatFilterExpressionToSearchBody:
    """Single entry point for the Langflow ``raw_search`` path (standalone contract)."""

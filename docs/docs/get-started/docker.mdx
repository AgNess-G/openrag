...
8. Optional: To enable HTTPS for the backend, place your SSL certificate and private key files in the `./keys` directory at the root of the repository, and then set the following variables in your `.env` file:
   ...
   Since the `./keys` directory is automatically mounted into the backend container, the server can access these files to serve traffic over HTTPS natively.

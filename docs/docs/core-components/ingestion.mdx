...
When ingesting from the local file system or cloud storage connectors, the **chat / local file picker** allows the following extensions only if they have been **verified** to ingest through the Langflow pipeline (same set is used for folder ingest filtering):
...
Other extensions (for example .mdx, .xml, code, or data files) are not offered in the local picker until ingestion support is confirmed.

Cloud connectors and other paths may support additional types (for example images or older Office formats). <!-- TBD: .vtt supported? -->
...
* The **Recent Tasks** section lists recently finished tasks.
   ...
   **Failed** means [something went wrong during ingestion](/support/troubleshoot#document-ingestion-or-similarity-search-issues), or the task was manually canceled.
   You can inspect the failed task to learn more.

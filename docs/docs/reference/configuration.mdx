...
| `SSL_KEYFILE` | Not set | Optional path to the SSL private key file (.pem) for the backend server. Enabling this and `SSL_CERTFILE` starts the backend server with native HTTPS support. |
| `SSL_CERTFILE` | Not set | Optional path to the SSL certificate file (.pem) for the backend server. Enabling this and `SSL_KEYFILE` starts the backend server with native HTTPS support. |
